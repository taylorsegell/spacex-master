...
palette = ['#9c4a3c', '#8C9B3C', '#d27702', '#ffe709',
           '#9e8d77', '#395112', '#9c4a3c', '#efddc9', '#206169']
...
completed_missions = important_df[important_df.upcoming ==
                                  False].flight_number.count()
upcoming_missions = important_df[important_df.upcoming ==
                                 True].flight_number.count()
landing_intent_missions = important_df[important_df.landing_intent ==
                                       True].flight_number.count()
land_success_missions = important_df[important_df.land_success ==
                                     True].flight_number.count()
...
def launch_sites(data):
    ...
    for year in unique_sites:
        ...
        launch_site_dict[year]['Launches'] = sites_df.loc[sites_df['Launch Site']
                                                          == year, 'Launch Site'].count()
        launch_site_dict[year]['Failures'] = sites_df.loc[(sites_df['Launch Site'] == year) & (
            sites_df.launch_site_success == False), 'launch_site_success'].count()
        launch_site_dict[year]['Success'] = sites_df.loc[(sites_df['Launch Site'] == year) & (
            sites_df.launch_site_success == True), 'launch_site_success'].count()
    launch_site_df = pd.DataFrame.from_dict(
        data=launch_site_dict, orient='index', columns=['launches', 'failures', 'success'])
    launch_site_df = pd.DataFrame(launch_site_dict).T
    launch_site_df['Percent Success'] = (
        (launch_site_df['Success']/(launch_site_df['Success']+launch_site_df['Failures']))*100).round(2)
...
fig1 = go.Figure(data=[
    go.Bar(
        ...
        marker_color='#9c4a3c',
        texttemplate='',
        hovertemplate='<i>Launches</i>: %{y}' +
        '<br><b>Year</b>: %{x}<br>'+'Percent Change: %<b>%{text:.2f}</b>',
...
customers_df = pd.DataFrame.from_dict(
    data=customers_dict, orient='index', columns=['Launches'])

fig2 = px.pie(customers_df, values='Launches', names=customers_df.index,
              title='SpaceX Customers and Number of Launches')
...
nations_df = pd.DataFrame.from_dict(
    data=nations_dict, orient='index', columns=['Nations'])
fig3 = px.pie(nations_df, values='Nations',
              names=nations_df.index, title='Nation Launches')
...
first_video_df = video_df.rename(
    {'Mission Name': 'label', 'Video Link': 'value'}, axis='columns')
...
video_dict = video_df.groupby('Launch Year').apply(
    lambda group: group.drop('Launch Year', axis=1).to_dict('records')).to_dict()
...
for item in manufacturers:
    orbits_dict[item] = {}
    for orbit in orbits:
        orbits_dict[item][orbit] = manufacturer_reference[(manufacturer_reference.manufacturer == item) & (
            manufacturer_reference.orbit == orbit)].orbit.count()
# Landing Tab-----------------------------------------------------------------------------------

app = dash.Dash(__name__, external_stylesheets=external_stylesheets,
                title='SpaceX Dashboard', update_title="3...2...1...", assets_folder="assets")
...
tab1 = dbc.Tab(label='Insights', children=[
    html.Div([
        html.Div([
            html.Div([
                html.H3("General Information", style={
                    'textAlign': 'center'}),
                html.Div(
                    [html.Div([html.P("Completed Missions: "), html.Div(completed_missions, className='info-num')], className='info'),
                     html.Div([html.P("Planned Missions Coming Up: "), html.Div(
                         upcoming_missions, className='info-num')], className='info'),
                     html.Div([html.P("Number of Misions with a Landing Intent: "), html.Div(
                         landing_intent_missions, className='info-num')], className='info'),
                     html.Div([html.P("Number of Missions with a Successful Land: "), html.Div(
                         land_success_missions, className='info-num')], className='info'),
                     ], className='info-div'),
            ], className='gen-info')
        ...
        html.Div(className='graph-grid', children=[
            ...
            html.Div(children=[
                dcc.RadioItems(id='cl_dropdown', options=soptions, value='Industry', style={
    ...
    html.Div([
        html.Div([
            html.Div([
                ...
                html.H4(),
                html.Div(children=[html.Div([html.P("CCAFS: "), html.Div(ccafs, className='info-num')], className='info'),
                                   html.Div([html.P("VAFB: "), html.Div(
                                       vafb, className='info-num')], className='info'),
                                   html.Div([html.P("KSC: "), html.Div(
                                       ksc, className='info-num')], className='info'),
                                   html.Div([html.P("Cape Canaveral: "), html.Div(
                                       capecav, className='info-num')], className='info'),
                                   html.Div([html.P("CCSFS: "), html.Div(
                                       ccsfs, className='info-num')], className='info'),
                                   html.Div([html.P("VSFB: "), html.Div(
                                       vsfb, className='info-num')], className='info'),
                                   ], className='info-div'),
            ], className='gen-info'),
        ]),
        ...
        html.Div(className='map_div', children=[
            html.Div(children=[
                ...
                html.Div(html.Iframe(id='map_fl', srcDoc=open('assets/florida_sites.html',
                         'r').read(), width='90%', height='500px', className='responsive-iframe')),
            ], className='frame-container'),
            html.Hr(),
            html.Div(children=[
                html.H4("California", className='header4'),
                html.Div(html.Iframe(id='map_ca', srcDoc=open('assets/cali_sites.html',
                         'r').read(), width='90%', height='500px)', className='responsive-iframe')),
            ], className='frame-container'),
        ]),
    ...
    html.Div(className='video-page', children=[
            ...
            html.Div(className='selection-box', children=[
                ...
                html.Div(className="mission_selection", children=[
                    html.Div(className='launch_year_dropdown', children=[
                        dbc.Label("Choose a Launch Year",
                                  html_for="launches_years"),
                        dcc.Dropdown(className='video-dropdown', id='launches_years', options=[
                            {'label': k, 'value': k} for k in video_dict.keys()], value='2016'),
                    ]),
                    html.Div(className='middle-space'),
                    html.Div(className='mission_dropdown', children=[
                        dbc.Label("Choose a Mission",
                                  html_for="missions"),
                        dcc.Dropdown(className='video-dropdown',
                                     id='missions', value='SES-9')
                    ]),
                ])
            ])
        ]),
        html.Div(className='lower-half', children=[
            html.Div(className='video-div', children=[
                html.H2('The Launch'),
                html.Iframe(className="mission_video",
                            id='frame', src=None)
            ]),
...
footer = html.Footer(id="footer",
                     ...
                     children=[html.A('Portfolio ', href='https://www.taylorsegell.com'),
                         ...
                         children=[
                             ...
                             html.Span('Data From:'),
                             html.A(
                                 'SpaceX API', href="https://github.com/r-spacex/SpaceX-API")
                         ])])
# App Construction-----------------------------------------------------------------------------------
...
def update_graph(site_dropdown):
    ...
    if (site_dropdown == 'All Sites'):
        piepalette = ['#8C9B3C', '#608A2E', '#8A7C2E',
                      '#c7c9a9', '#9B703C', '#9c4a3c']
        lsdf = data.groupby(['Launch Site']).agg(
            {'Class': ['mean', 'sum', 'count']})
        lsdf['Perecent Success'] = lsdf[(
            'Class',   'sum')]/lsdf[('Class',   'sum')].sum()*100
        lsdf['Failed Missions'] = lsdf[(
            'Class',   'count')]-lsdf[('Class',   'sum')]
        lsdf['Perecent Failure'] = lsdf['Failed Missions'] / \
            lsdf['Failed Missions'].sum()*100
        ...
        fig2.update_traces(
            hovertemplate='Launch Site: %{label}<br>Failed/Total Missions: %{customdata}<br>Successful Missions: %{value}')
...
def update_graph(site_dropdown, cl_dropdown):
    if site_dropdown == 'All Sites':
        if cl_dropdown == ['Industry']:
            df = data
            con = df.groupby(['Ownership', 'Continent', 'Industry', 'Country', 'Company']).agg(
                {'Flight No.': 'count', 'Class': 'mean'})
            cont = pd.DataFrame(con)
            cont = cont.reset_index()

            fig = px.sunburst(cont, path=['Ownership', cl_dropdown, 'Company'], values='Flight No.',
                              color='Class', color_continuous_scale=[(0, '#9c4a3c'), (1, '#8C9B3C')])
            fig.update_traces(
                hovertemplate='<b>%{label} </b> <br> Flights: %{value}<br> Success Rate: %{color:.2f}')
            fig.update_layout(title='Launch Success By ' +
                              cl_dropdown+' at '+site_dropdown, title_x=.5)
        else:
            df = data
            con = df.groupby(['Ownership', 'Continent', 'Industry', 'Country', 'Company']).agg(
                {'Flight No.': 'count', 'Class': 'mean'})
        ...
        if cl_dropdown == ['Industry']:
            ...
            df = data.loc[data['Launch Site'] == site_dropdown]
            con = df.groupby(['Ownership', 'Continent', 'Industry', 'Country', 'Company']).agg(
                {'Flight No.': 'count', 'Class': 'mean', 'Payload Mass(kg)': 'mean'})
            cont = pd.DataFrame(con)
            cont = cont.reset_index()
            fig = px.sunburst(cont, path=['Ownership', cl_dropdown, 'Company'], values='Flight No.',
                              color='Class', color_continuous_scale=[(0, '#9c4a3c'), (1, '#8C9B3C')])
...
def update_barchart(site_dropdown):
    def launch_fun(data):
        ...
        for year in unique_years:
            launch_dict[year] = {}
            launch_dict[year]['Launches'] = year_fail_df.loc[year_fail_df.Years ==
                                                             year, 'Years'].count()
            launch_dict[year]['Failures'] = year_fail_df.loc[(year_fail_df.Years == year) & (
                year_fail_df.launch_success == False), 'launch_success'].count()
            launch_dict[year]['Success'] = year_fail_df.loc[(year_fail_df.Years == year) & (
                year_fail_df.launch_success == True), 'launch_success'].count()
        launch_df = pd.DataFrame.from_dict(data=launch_dict, orient='index', columns=[
                                           'launches', 'failures', 'success'])
        launch_df = pd.DataFrame(launch_dict).T
        launch_df['Percent Change(Success)'] = launch_df['Success'].pct_change().round(
            4)*100
        launch_df['Percent Change(Failure)'] = launch_df['Failures'].pct_change().round(
            4)*100
        return launch_df
    ...
    if site_dropdown == 'All Sites':
        ...
        fig1.update_layout(
            barmode='stack',
            title='Success/Failures in Launch<br> Site: ' +
            site_dropdown+'(2010-2022)',
            title_x=.5,
            uniformtext_mode='hide'
        )
    return fig1
